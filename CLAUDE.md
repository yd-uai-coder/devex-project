...
**#24. decision digest の更新タイミングと置き場**(所感5.2対応)。decision digest は `textbook/decision-digest.md` に置く。`q_a.md`(#8、保存専用・非参照)とは異なり、decision digest は**進行中に能動的に参照する**ファイルである。追記は Phase 完了時にまとめて 1 回行う(生成セッション中に都度追記しない)。これは #18 のセッション分離と整合させる。**decision digest は Devex というこのアプリケーション自身で完結する決定・知見に限定する**(Devex の外側、他リポジトリに関わる事項は書かない)。他リポジトリに関わる事項は #32 の `retrospective-memo.md` へ。
...
**#29. サンプルファイルのPhase履歴表記の具体形式**(#12の具体化。Phase 2で確立)。#12が定める「新規はファイル冒頭コメントに出自Phaseを明記」「更新は旧コードをコメントアウトし新コードに`# (Phase <N>-<M>)`タグ」を、`textbook/samples/`配下で機械的に運用できるよう以下の具体形式に統一する。

- **ファイル冒頭ヘッダー**: 各ファイルの先頭行に `# 作成：Phase-<N>-<n>` および/または `# 更新：Phase-<M>-<m>,<O>-<o>`(複数章にまたがる場合はカンマ区切りで列挙)を1行で記す。両方ある場合は `# 作成：Phase-<N>-<n>｜更新：Phase-<M>-<m>` のように`｜`で連結する。CL開発以前のスターターテンプレート由来のファイル(モデル定義前から存在する`devex-api`の既存ファイル等)は「作成」を省略し「更新」のみ記す。
- **変更箇所のタグ(3種)**:
  - 追記(既存コードに影響を与えない純粋な追加): 追加箇所の直前に `# Phase-<N>-<n>:追記` を1行。
  - 更新(既存コードの変更): `# Phase-<N>-<n>：更新` の下に旧コードを`#`でコメントアウトし、`# ↓↓` を挟んで新コードを続ける。
  - 削除: `# Phase-<N>-<n>：削除` の下に旧コードを`#`でコメントアウトするのみ(新コードは続けない)。
...
- 同一階層で依存関係が無いファイル同士は、層別の慣習(モデル→リポジトリ→サービス→ルート)を保つ。
- **テストはまとめて表の末尾に置く**(実装ファイル群をすべて依存順に並べ終えた後に、テスト群を続ける。実装の直後にそのテストを挟まない)。テスト用フィクスチャ(`tests/fixtures/*.py`)もテスト群の先頭に含める。テスト群内部の順序は、依存する実装ファイルが既に表の上側に揃っているため自由でよい。
- 対象は`textbook/samples/`を持つ実装Phase全般(設計フェーズは対象外)。

**#31. 章冒頭の「この章の目的」節**(本プロジェクト固有)。各`Phase-<N>-<n>.md`(作業単位の章ファイル)は、タイトル直後・モード宣言(#21の学習モード/納期モード表記)より前に`## この章の目的`節を置き、当該章で実装する機能の概要(何を・なぜ実装するか)を1〜数文で説明する。`Phase-<N>-introduction.md`の章一覧(#6)が章間の位置づけ・依存関係を俯瞰するのに対し、この節は章ファイル単体を開いた時点で「この章で何を作るか」が分かるようにするためのもの。既存の設計判断・実装内容を変更するものではなく、既存内容からの要約でよい。

**#32. 振り返り用覚書ログ**(本プロジェクト固有)。`textbook/retrospective-memo.md`は、プロジェクト完了後の振り返り(#10)で見返すための汎用の覚書ログである。decision digest(#24)が「Devex自身で完結する決定・知見」に限定されるのに対し、こちらは「Devexの外側に関わる情報」(`devex-api`/`devex-ui`がテンプレートリポジトリでもあることへの反映候補など)を置く。各エントリの先頭に種別タグ(例: `[テンプレート反映候補]`)を付け、複数の種別を同じファイルに集約する。該当する気づきがあった都度追記してよい(decision digestのような Phase 完了時一括方式は取らない)。エントリ形式: (1) 種別タグ (2) 発生した Phase・章 (3) 内容の概要 (4) 判断理由。`q_a.md`(#8)同様、保存専用に近く作業中は参照しない。
